...
#表达矩阵
exp_gtex.tpm=fread("data/pancancer/gtex_RSEM_gene_tpm.gz",header = T, sep = '\t',data.table = F)
rownames(exp_gtex.tpm)=exp_gtex.tpm[,1]
exp_gtex.tpm=exp_gtex.tpm[,-1]

#样本信息
data_cl=fread("data/pancancer/GTEX_phenotype.gz",header = T, sep = '\t',data.table = F)
data_cl=data_cl[,c(1,3)]
names(data_cl)=c('Barcode','Tissue')
data_cl=data_cl[data_cl$Tissue == 'Ovary',] #筛选出ovarian的数据

#注释信息
annotat=fread("data/pancancer/gencode.v23.annotation.gene.probemap",header = T, sep = '\t',data.table = F)
