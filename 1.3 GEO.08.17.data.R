...
###
expr <- GSE40595.stroma.exprSet
group <- GSE40595.stroma.clin[,1,drop=F]

module.gene <- intersect(rownames(expr),gene$`Gene Symbol`)

expr <- as.data.frame(expr[module.gene,])
Group <- data.frame("id"=colnames(expr),
                    "Group" = ifelse(group$Group=="Normal ovarian stroma","Normal","Cancer"))
...
### 4.输出差异分析结果,其中coef的数目不能操过design的列数
### 此处的2代表的是design中第二列和第一列的比较
GSE40595.stroma.allDiff=topTable(fit2,adjust='fdr',coef=2,number=Inf)
...
GSE18520 <- GSE18520.allDiff %>%
            ...
            select(ID,logFC)
GSE40595.stroma <- GSE40595.stroma.allDiff %>%
  rownames_to_column("ID") %>%
  select(ID,logFC)
...
logfc <- GSE18520 %>%
   ...
   full_join(.,GSE40595.stroma,by ="ID")%>%
   column_to_rownames("ID")
colnames(logfc) <- c("GSE18520",
                     "GSE40595",
                     "GSE40595.stroma")
#####################
GSE18520.p <- GSE18520.allDiff %>%
  rownames_to_column("ID") %>%
  select(ID,adj.P.Val)
GSE40595.stroma.p <- GSE40595.stroma.allDiff %>%
  rownames_to_column("ID") %>%
  select(ID,adj.P.Val)
...
p.val <- GSE18520.p %>%
  ...
  full_join(.,GSE40595.stroma.p,by ="ID")%>%
  column_to_rownames("ID")
colnames(p.val) <- c("GSE18520",
                     "GSE40595",
                     "GSE40595.stroma")
